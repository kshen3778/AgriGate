...
def moisture(c1, c2):
    ...
    cur.execute('SELECT * FROM moisturel4 WHERE lat BETWEEN %s AND %s AND lng BETWEEN %s AND %s', [min(lat1,lat2),max(lat1,lat2),min(lng1,lng2),max(lng1,lng2)])
...
def vegetation(c1, c2):
    ...
    lat1, lng1 = map(float, c1)
    lat2, lng2 = map(float, c2)
    cur.execute('SELECT * FROM vegetationl4 WHERE lat BETWEEN %s AND %s AND lng BETWEEN %s AND %s', [min(lat1,lat2),max(lat1,lat2),min(lng1,lng2),max(lng1,lng2)])
...
def soilloose(c1,c2):
    ...
    lat1, lng1 = map(float, c1)
    lat2, lng2 = map(float, c2)
    cur.execute('SELECT * FROM loose_soil WHERE lat BETWEEN %s AND %s AND lng BETWEEN %s AND %s', [min(lat1,lat2),max(lat1,lat2),min(lng1,lng2),max(lng1,lng2)])
    loose = cur.fetchall()
    return {'loose': loose}
